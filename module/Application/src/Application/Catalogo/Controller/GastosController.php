...
class GastosController extends AbstractActionController
{
        ...
        if($request->isPost()){
            ...
            */
            ...
            if(!empty($post_data['search']['value'])){
                ...
                $c1= $c->getNewCriterion('gasto.idgasto', '%'.$search_value.'%', \Criteria::LIKE);
                $c2= $c->getNewCriterion('gasto.gasto_nombre', '%'.$search_value.'%', \Criteria::LIKE);

                 $c3= $c->getNewCriterion('gasto.gasto_descripcion', '%'.$search_value.'%', \Criteria::LIKE);

          
                $c1->addOr($c2)->addOr($c3);

                $query->addAnd($c1);
                $query->groupByIdgasto();
            ...
            foreach ($query->find()->toArray(null,false,  \BasePeer::TYPE_FIELDNAME) as $value){
                ...
                $tmp['DT_RowId'] = $value['idgasto'];
                $tmp['idgasto'] = $value['idgasto'];
                $tmp['gasto_nombre'] = $value['gasto_nombre'];
                $tmp['gasto_descripcion'] = $value['gasto_descripcion'];
                $tmp['options'] = '<td><div class="btn-group dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li>
                      <a href="/catalogo/gastos/ver/' . $value['idgasto'] . '">
                        <div class="media">
    ...
    public function indexAction()
    {
        ...
        $view_model->setTemplate('application/catalogo/gastos/index');
    ...
    public function nuevoAction()
    {
        ...
        if($request->isPost())
        {
            ...
            foreach ($post_data as $key => $value) {
                if(\GastoPeer::getTableMap()->hasColumn($key))
                {
            ...
            return $this->redirect()->toUrl('/catalogo/gastos');
        }


        $form = new \Application\Catalogo\Form\GastosForm();
        $view_model = new ViewModel();
        $view_model->setTemplate('application/catalogo/gastos/nuevo');
        $view_model->setVariables(array(
            'form' => $form
        ));
    ...
    public function verAction()
    {
        ...
        $exists = \GastoQuery::create()->filterByIdgasto($id)->exists();

        if($exists)
        {
            $entity = \GastoQuery::create()->findPk($id);
            ...
            $view_model = new ViewModel();
            $view_model->setTemplate('application/catalogo/gastos/ver');
