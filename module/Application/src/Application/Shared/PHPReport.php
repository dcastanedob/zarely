...
class PHPReport {
    ...
    public function __construct($config=array())
	{
        ...
        $document_root = explode('/index.php', $_SERVER['SCRIPT_NAME'])[0];
        if($document_root == ""){
     ...
     */
    public function render($type='html',$filename='',$base_64 = false, $path_to_save = false)
    {
		...
		elseif(strtolower($type)=='excel2003')
			return $this->renderXls($filename,$base_64,$path_to_save);
		elseif(strtolower($type)=='pdf')
			return $this->renderPdf($filename,$base_64);
		else
	 ...
	 */
	private function renderXls($filename,$base_64 = false,$path_to_save = false)
    {            
		...
		$this->objWriter = PHPExcel_IOFactory::createWriter($this->objPHPExcel, 'Excel5');
                 ...
                 if(!$base_64){
                    ...
                    if(!$path_to_save){
                        ...
                        $this->objWriter->save($this->document_root.'/base_64.xls');
                        return base64_encode(file_get_contents($this->document_root.'/base_64.xls'));
                        
                    }else{
                        
                        $fp = fopen($path_to_save.'/'.$filename.'.xls', 'w');
                        fclose($fp);
                        chmod($path_to_save.'/'.$filename.'.xls', 0777); 
                        $this->objWriter->save($path_to_save.'/'.$filename.'.xls');
                        return true;
	 ...
	 */
	private function renderPdf($filename,$base_64 = false)
	{
		...
		$this->objWriter = PHPExcel_IOFactory::createWriter($this->objPHPExcel, 'PDF');
                ...
                if(!$base_64){
                    ...
                    $this->objWriter->save($_SERVER['DOCUMENT_ROOT'].'/base_64.pdf');
                    return base64_encode(file_get_contents($_SERVER['DOCUMENT_ROOT'].'/base_64.pdf'));
                 }
