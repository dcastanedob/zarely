...
abstract class BaseCuentabancariamovimientoQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterByCuentabancariamovimientoReferencia('fooValue');   // WHERE cuentabancariamovimiento_referencia = 'fooValue'
     * $query->filterByCuentabancariamovimientoReferencia('%fooValue%'); // WHERE cuentabancariamovimiento_referencia LIKE '%fooValue%'
     * </code>
     *
     * @param     string $cuentabancariamovimientoReferencia The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return CuentabancariamovimientoQuery The current query, for fluid interface
     */
    public function filterByCuentabancariamovimientoReferencia($cuentabancariamovimientoReferencia = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($cuentabancariamovimientoReferencia)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $cuentabancariamovimientoReferencia)) {
                $cuentabancariamovimientoReferencia = str_replace('*', '%', $cuentabancariamovimientoReferencia);
                $comparison = Criteria::LIKE;
            }
        }
