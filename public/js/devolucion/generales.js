        ...
        */ 
        ...
        calcularTotal = function(){
          ...
          $container.find('#tables_information table tbody tr').filter(function(){
            subtotal = accounting.unformat($(this).find('input.devoluciondetalle_subtotal').val());
            total+= parseFloat(subtotal);

          });
          $container.find('input[name=devolucion_total]').val(accounting.formatMoney(total));
        ...
        calcularSubtotal = function($row){
          ...
          $row.find('input.devoluciondetalle_subtotal').val(accounting.formatMoney(subtotal));
          calcularTotal();
        }
        ...
        plugin.initForm = function(){
            ...
            $container.find('input[name=devolucion_fecha]').datepicker({
                language:'es',
            });
            ...
            $.ajax({
                url:'/devoluciones/generales/getProductosvariantes',
                type: 'POST',
                dataType: 'JSON',
                data:{
             ...
             var productovariantes_text =  $container.find('select[name*=idproductovariante]').multipleSelect("getSelects", "text");
              ...
              productovariantes_text.forEach(function(value,index){
                 ...
                 if($.inArray(id,productosvariantes_added) < 0){
                  ...
                  $.ajax({
                    method: 'POST',
                    url:'/devoluciones/generales/initializetable',
                    dataType:'JSON',
                    data:{
                      id:id,
                      ...
                      $.each(data.data,function(index, tables){
                        $.each(tables,function(indexRango, value){
                          ...
                          $tr.append('<td><input type="text" size="5" class="devoluciondetalle_subtotal" value="0.0" disabled></input></td>');
            ...
            $container.find('#btn_add_productogeneral').on('click',function(){
              ...
              $.ajax({
                  method: 'POST',
                  url:'/devoluciones/generales/getdetails',
                  dataType:'JSON',
                  data:{
                      ...
                      $tr.find('button').on('click',function(){
                          ...
                          $tr.find('input[name=variantes]').filter(function(){
                            if($(this).val() != "")
                            {
                              ...
                              $.ajax({
                                method: 'POST',
                                url:'/devoluciones/generales/initializetablegeneral',
                                dataType:'JSON',
                                data:{
                                  id:id,
                                },success:function(data){
